...
class Batch:
    ...
    def index_select(batch, indices):
        ...
        for key, examples in batch.items():
            if key == "anchored_labels":
                filtered_batch[key] = [examples[i] for i in indices] #[[example[i] for i in indices] for example in examples]
            else:
                if isinstance(examples, list) or isinstance(examples, tuple):
                    try:
                        ...
                        print(f"Failed to collect field {key}")
                else:
                    try:
