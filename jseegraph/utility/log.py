...
class Log:
    ...
    def log_evaluation(self, scores, mode, epoch, framework, language):
        f1_score = scores['trigger_classification'][-1]
        if self.log_wandb:
            metrics = ['precision', 'recall', 'f1']
            scores = {f"{mode}-{framework}-{language}/{k}/{metrics[i]}":v[i] for k,v in scores.items() for i in range(len(v))}
