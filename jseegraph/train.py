...
def parse_arguments():
    ...
    parser.add_argument("--validate_each", type=int, default=1, help="Validate every ${N}th epoch.")
...
def main(directory, args):
    ...
    #    loss_weight_learner = LossWeightLearner(args, model)
        ...
        for i, batch in tqdm(enumerate(dataset.train)):
            ...
            if (i + 1) % args.accumulation_steps == 0:
                ...
                with torch.no_grad():
                    ...
                    log(batch_size, stats, args.frameworks, grad_norm=grad_norm, learning_rates=scheduler.lr())
    ...
    predict(model, dataset.test, args.test_data, args.raw_test_data, args, None, f"{directory}/test_predictions", device, mode="test")
...
if __name__ == "__main__":
    ...
    directory = f"{args.output_directory}/{args.name}_{timestamp}"
