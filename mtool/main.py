...
def read_graphs(stream, format = None,
  ...
  if format in {"amr", "camr"}:
    ...
    generator = codec.evt_ent.read(stream, text=text)
  elif format == "ent":
    generator = codec.ent.read(stream, text=text)  
  elif format == "pmb":
...
def main():
  ...
  if arguments.write is not None and \
     ...
     {"dot", "tikz", "displacy", "evaluation", "id", "json", "mrp",
      "source", "targets", "txt", "ucca", "norec", "ace", "evt-ent", "ent"}:
  ...
  if arguments.write == "ace":
    ...
    json.dump(ace_graphs, arguments.output, indent=None, ensure_ascii=False)

  if arguments.write == "evt-ent":
    evt_ent_graphs = [codec.evt_ent.write(graph, graph.input) for graph in graphs]
    json.dump(evt_ent_graphs, arguments.output, indent=None, ensure_ascii=False)

  if arguments.write == "ent":
    ent_graphs = [codec.ent.write(graph, graph.input) for graph in graphs]
    json.dump(ent_graphs, arguments.output, indent=None, ensure_ascii=False)
